...
def create_train_step_fn(task, network, loss_fn, optimizer, loss_ratio, scaler, reducer,
    ...
    from mindspore.amp import all_finite as _ms_all_finite
    # CPU 后端不支持 all_finite 内部用到的 get_tensor 算子，这里以恒 True 占位（仅本地调试用，Ascend/GPU 路径不变）
    if ms.get_context("device_target") == "CPU":
        def all_finite(grads):
            return ms.Tensor(True, ms.bool_)
    else:
        all_finite = _ms_all_finite
